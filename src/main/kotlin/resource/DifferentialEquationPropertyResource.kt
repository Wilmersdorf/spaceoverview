...
class DifferentialEquationPropertyResource @Inject constructor(
    ...
    @GET
    fun get(): Response {
        val differentialEquationProperties =
            differentialEquationPropertyDao.getAll().sortedWith(compareBy(String.CASE_INSENSITIVE_ORDER) { it.name })
    ...
    @POST
    fun add(postDto: PostDifferentialEquationPropertyDto, @Auth user: User): Response {
        ...
        if (errors.isNotEmpty()) {
            ...
            val references =
                toDataMapper.toDifferentialEquationPropertyReferences(
    ...
    fun update(
        ...
        postDto: PostDifferentialEquationPropertyDto,
        @Auth user: User
    ): Response {
        val differentialEquationProperty = differentialEquationPropertyDao.get(id)
        val errors = validate(postDto)
